...
impl Db {
    ...
    ///
    /// This is the Design-B seam used by `buzz-relay`'s NIP-FI atomic path to
    /// keep the event insert inside the same SERIALIZABLE transaction as the
    /// admission authority writes.  The caller owns `BEGIN`, isolation level,
    /// and `COMMIT`/`ROLLBACK` — this function only executes the insert rows.
    ///
