...
/// Result-level read authorization for events whose kind is recipient-private.
/// For every [`crate::kind::P_GATED_KINDS`] event, the reader MUST equal one of
/// the event's `#p` tags. Returns `true` for every other kind.
///
