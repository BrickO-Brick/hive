...
use buzz_db::workflow::{ApprovalStatus, RunStatus, WorkflowRecord};
use buzz_db::DbError;
...
/// Outcome counts from one legacy workflow reconciliation pass.
#[derive(Debug, Default, PartialEq, Eq)]
pub struct LegacyWorkflowReconcileReport {
    /// Legacy workflow rows examined.
    pub examined: usize,
    /// Rows bound to an exact signed definition.
    pub bound: usize,
    /// Rows left on the compatibility fallback because provenance was incomplete.
    pub unprovable: usize,
    /// Rows concurrently bound by another writer.
    pub raced: usize,
}

fn legacy_definition_matches(workflow: &WorkflowRecord, event: &Event) -> bool {
    let workflow_id = workflow.id.to_string();
    let workflow_channel_id = workflow.channel_id.map(|channel| channel.to_string());
    if event.kind.as_u16() as u32 != KIND_WORKFLOW_DEF
        || event.pubkey.to_bytes().as_slice() != workflow.owner_pubkey.as_slice()
        || extract_d_tag(event).as_deref() != Some(workflow_id.as_str())
        || extract_h_tag(event).as_deref() != workflow_channel_id.as_deref()
    {
        return false;
    }

    let Ok((_, canonical_json)) = buzz_workflow::WorkflowEngine::parse_yaml(&event.content) else {
        return false;
    };
    let Ok(mut definition) = serde_json::from_str::<serde_json::Value>(&canonical_json) else {
        return false;
    };

    // Webhook secrets were generated by the relay and never existed in the
    // owner-signed event. Reuse the stored secret before comparing the exact
    // materialized JSON and its legacy hash.
    if let Some(secret) = webhook_secret::extract_secret(&workflow.definition) {
        webhook_secret::inject_secret(&mut definition, &secret);
    }
    let Ok(materialized_json) = serde_json::to_string(&definition) else {
        return false;
    };

    definition == workflow.definition
        && compute_definition_hash(&materialized_json) == workflow.definition_hash
}

/// Reconcile legacy workflow rows only when the live signed NIP-33 head proves
/// the exact materialized definition. Historical runs deliberately remain NULL:
/// the current head cannot prove which earlier revision a completed run used.
pub async fn reconcile_legacy_workflow_revisions(
    db: &buzz_db::Db,
) -> Result<LegacyWorkflowReconcileReport, DbError> {
    let workflows = db.list_legacy_workflows().await?;
    let mut report = LegacyWorkflowReconcileReport {
        examined: workflows.len(),
        ..Default::default()
    };
    ...
    for workflow in workflows {
        ...
        let mut events = db.query_events(&query).await?;
        let Some(stored) = events.pop() else {
            report.unprovable += 1;
            continue;
        };
        if !legacy_definition_matches(&workflow, &stored.event) {
            report.unprovable += 1;
            continue;
        }
...
mod tests {
    ...
    fn legacy_workflow_for_event(event: &Event, channel_id: Uuid) -> WorkflowRecord {
        let (_, canonical_json) = buzz_workflow::WorkflowEngine::parse_yaml(&event.content)
            .expect("canonical workflow definition");
        let definition: serde_json::Value =
            serde_json::from_str(&canonical_json).expect("definition json");
        let materialized_json =
            serde_json::to_string(&definition).expect("production materialized json");
        WorkflowRecord {
            id: Uuid::parse_str(extract_d_tag(event).as_deref().expect("d tag"))
                .expect("workflow UUID"),
            community_id: CommunityId::from_uuid(Uuid::new_v4()),
            name: "legacy".to_string(),
            owner_pubkey: event.pubkey.to_bytes().to_vec(),
            channel_id: Some(channel_id),
            definition_hash: compute_definition_hash(&materialized_json),
            definition,
    ...
    #[test]
    fn legacy_revision_match_requires_exact_semantic_provenance() {
        ...
        let channel_id = Uuid::parse_str(extract_h_tag(&event).as_deref().expect("h tag"))
            .expect("channel UUID");
        let workflow = legacy_workflow_for_event(&event, channel_id);
        assert!(legacy_definition_matches(&workflow, &event));

        let mut wrong_hash = workflow.clone();
        wrong_hash.definition_hash = vec![0x55; 32];
        assert!(!legacy_definition_matches(&wrong_hash, &event));

        let mut wrong_channel = workflow.clone();
        wrong_channel.channel_id = Some(Uuid::new_v4());
        assert!(!legacy_definition_matches(&wrong_channel, &event));

        let mut wrong_owner = workflow;
        wrong_owner.owner_pubkey = Keys::generate().public_key().to_bytes().to_vec();
        assert!(!legacy_definition_matches(&wrong_owner, &event));
    }

    #[test]
    fn legacy_webhook_secret_is_reapplied_before_matching() {
        let keys = Keys::generate();
        let workflow_id = Uuid::new_v4();
        ...
        .sign_with_keys(&keys)
        .expect("workflow event");
        let mut workflow = legacy_workflow_for_event(&event, channel_id);
        webhook_secret::inject_secret(&mut workflow.definition, "legacy-secret");
        workflow.definition_hash = compute_definition_hash(
            &serde_json::to_string(&workflow.definition).expect("materialized definition"),
        );
    ...
    async fn reconciliation_finds_historical_null_channel_event_and_binds_it() {
        ...
        let (_, canonical_json) = buzz_workflow::WorkflowEngine::parse_yaml(content)
            .expect("canonical workflow definition");
        let definition: serde_json::Value =
            serde_json::from_str(&canonical_json).expect("production definition value");
        let definition_json_final =
            serde_json::to_string(&definition).expect("production materialized definition");
        let workflow_id = db
            .create_workflow(
                tenant.community(),
                Some(channel.id),
                ...
                "legacy",
                &definition_json_final,
                &compute_definition_hash(&definition_json_final),
            )
            .await
