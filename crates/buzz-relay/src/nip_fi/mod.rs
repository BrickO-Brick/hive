...
impl<S: IssuerKeySource + Sync + Send + 'static> NipFiVerify for NipFiVerifierImpl<S> {
    ...
    async fn commit_kind9_atomic(
        ...
        // ── Assertion revalidation (before any transaction) ───────────────
        // Revalidate the compact JWS once, outside the SERIALIZABLE window.
        // This keeps the network round-trip out of the transaction and makes
        // commit_admission_in_tx testable with a synthetic VerifiedAssertion.
        // db_now is approximated here with Utc::now(); the authoritative
        // transaction_timestamp() check happens inside the transaction.
        let fresh_assertion =
            admission::revalidate_assertion(&*verifier, &verified_assertion, chrono::Utc::now())?;
