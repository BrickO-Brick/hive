...
/// `pub` version exposed only when the `nip-fi-boundary-test` feature is
/// active.  This dissolves the struct visibility wall so compile-fail fixtures
/// in `buzz-nip-fi-inner-seal-test` can name the type directly and observe
/// E0624 on `seal_inline` (not an earlier "not publicly re-exported" error).
/// Never enabled in production builds.
#[cfg(feature = "nip-fi-boundary-test")]
pub struct SealedRequestContext {
    /// Nostr-proof transport that bound the actor.
    pub(super) transport: ProofTransport,
    /// Full 32-byte event ID of the NIP-42 AUTH or NIP-98 proof event.
    pub(super) proof_event_id: [u8; 32],
...
impl SealedRequestContext {
    ...
    /// auth-handshake path.  The compile-fail fixtures in `buzz-nip-fi-seal-test`
    /// prove the outer module wall; this docstring is the intra-crate contract.
