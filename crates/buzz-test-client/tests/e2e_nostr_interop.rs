...
/// Send a message with an explicit supported kind and return the event id.
...
/// Every accepted human-visible message kind resurfaces a hidden DM for the
/// recipient while preserving the sender and unrelated conversations.
#[tokio::test]
#[ignore]
async fn test_nipdv_supported_message_kinds_resurface_hidden_dm_for_recipient() {
    for kind in [9u16, 40002, 45001, 45003] {
        ...
        for (keys, dm_id) in [
            (&keys_a, channel_id.as_str()),
            (&keys_b, channel_id.as_str()),
            (&keys_b, unrelated_channel_id.as_str()),
        ] {
            post_signed_event(keys, 41012, vec![Tag::parse(["h", dm_id]).unwrap()]).await;
        }
        ...
        assert!(
            ...
            "kind {kind} must preserve the sender's hidden state; A sees: {a_hidden:?}"
        );
        client_a.disconnect().await.expect("disconnect A");
        ...
        assert!(
            ...
            "kind {kind} must resurface the DM for B; B sees: {b_hidden:?}"
        );
        assert!(
            b_hidden.contains(&unrelated_channel_id),
            "kind {kind} must preserve unrelated hidden DMs; B sees: {b_hidden:?}"
        );
        client_b.disconnect().await.expect("disconnect B");
    }
}

/// Group-DM delivery resurfaces every hidden recipient, not only the first.
#[tokio::test]
#[ignore]
