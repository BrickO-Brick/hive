//! OS keyring access for Buzz nsec private keys.
//!
...
//! and reads return [`ReadonlySecretError::Disabled`]. Desktop may then apply
//! its own file-storage policy; the CLI does not.
//!
...
/// Keychain service used by signed Buzz Desktop releases.
pub const DESKTOP_RELEASE_SERVICE: &str = "buzz-desktop";

/// Key within the Desktop secret blob that holds the user's Nostr identity.
pub const DESKTOP_IDENTITY_KEY: &str = "identity";

/// A redacted failure reading a secret without changing secure storage.
#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
pub enum ReadonlySecretError {
    /// The OS credential store is locked or otherwise inaccessible this boot.
    #[error("secure storage is locked or unavailable")]
    LockedOrUnavailable,
    /// The user or an access-control policy denied this process access.
    #[error("secure storage access was denied")]
    AccessDenied,
    /// The stored blob cannot be decoded using Buzz's JSON-map schema.
    #[error("secure storage entry is corrupt")]
    Corrupt,
    /// The platform credential backend failed for another reason.
    #[error("secure storage is unavailable")]
    Unavailable,
    /// This build does not include an OS credential-store backend.
    #[error("system keyring support is disabled")]
    Disabled,
}
...
/// Read one value from a serialized Buzz secret blob.
#[cfg(any(feature = "system-keyring", test))]
fn decode_readonly_secret(raw: String, key: &str) -> Result<Option<String>, ReadonlySecretError> {
    let raw = Zeroizing::new(raw);
    let mut map = serde_json::from_str::<HashMap<String, String>>(&raw)
        .map_err(|_| ReadonlySecretError::Corrupt)?;
    let requested = map.remove(key);
...
fn classify_platform_read_failure(
    error: &(dyn std::error::Error + Send + Sync + 'static),
) -> ReadonlySecretError {
    let Some(error) = error.downcast_ref::<security_framework::base::Error>() else {
        return ReadonlySecretError::Unavailable;
    };
    match error.code() {
        // errSecAuthFailed and errSecUserCanceled.
        -25293 | -128 => ReadonlySecretError::AccessDenied,
        // errSecInteractionNotAllowed and inaccessible-keychain errors.
        -25308 | -25291 | -25292 | -25294 | -25295 => ReadonlySecretError::LockedOrUnavailable,
        _ => ReadonlySecretError::Unavailable,
    }
}
...
fn classify_platform_read_failure(
    ...
    ReadonlySecretError::Unavailable
}

#[cfg(feature = "system-keyring")]
fn classify_read_error(error: &keyring::Error) -> ReadonlySecretError {
    match error {
        keyring::Error::NoStorageAccess(_) => ReadonlySecretError::LockedOrUnavailable,
        keyring::Error::BadEncoding(_) => ReadonlySecretError::Corrupt,
        keyring::Error::PlatformFailure(error) => classify_platform_read_failure(error.as_ref()),
        _ => ReadonlySecretError::Unavailable,
    }
}

/// Read a secret from the current JSON blob without migration or mutation.
///
/// This path never writes, deletes, probes legacy entries, or falls back to a
/// file. It is intended for consumers such as the CLI that may borrow a
/// Desktop identity but must not participate in Desktop's persistence policy.
pub fn load_secret_readonly(
    service: &str,
    key: &str,
) -> Result<Option<String>, ReadonlySecretError> {
    #[cfg(feature = "system-keyring")]
    {
        let entry =
            keyring_entry(service, BLOB_KEY).map_err(|_| ReadonlySecretError::Unavailable)?;
        match entry.get_password() {
            Ok(raw) => decode_readonly_secret(raw, key),
    ...
    {
        let _ = (service, key);
        Err(ReadonlySecretError::Disabled)
    }
}

/// Read the identity stored by the signed Buzz Desktop release.
///
/// This function is strictly read-only. In particular, it does not inspect or
/// migrate old per-key entries and does not use Desktop's plaintext fallback.
pub fn load_desktop_release_identity() -> Result<Option<String>, ReadonlySecretError> {
    load_secret_readonly(DESKTOP_RELEASE_SERVICE, DESKTOP_IDENTITY_KEY)
}
...
impl SecretStore {
    ...
    /// Read `key` from this store's current blob without migration or mutation.
    pub fn load_readonly(&self, key: &str) -> Result<Option<String>, ReadonlySecretError> {
        load_secret_readonly(&self.service, key)
    }
...
mod readonly_tests {
    ...
    #[test]
    fn readonly_decode_returns_requested_secret() {
        let raw = r#"{"identity":"nsec1example","agent:abc":"other-secret"}"#.to_string();
        assert_eq!(
            decode_readonly_secret(raw, DESKTOP_IDENTITY_KEY).unwrap(),
            Some("nsec1example".to_string())
        );
    }

    #[test]
    fn readonly_decode_reports_missing_secret_without_error() {
        let raw = r#"{"agent:abc":"other-secret"}"#.to_string();
        assert_eq!(
            decode_readonly_secret(raw, DESKTOP_IDENTITY_KEY).unwrap(),
            None
        );
    ...
    #[test]
    fn readonly_decode_redacts_corrupt_blob() {
        let secret = "nsec1must-not-appear";
        let error =
            decode_readonly_secret(format!(r#"{{"identity":"{secret}""#), "identity").unwrap_err();
        assert_eq!(error, ReadonlySecretError::Corrupt);
        assert!(!error.to_string().contains(secret));
    }

    #[test]
    fn desktop_storage_coordinates_are_stable() {
        assert_eq!(DESKTOP_RELEASE_SERVICE, "buzz-desktop");
        assert_eq!(DESKTOP_IDENTITY_KEY, "identity");
    }

    #[cfg(all(feature = "system-keyring", target_os = "macos"))]
    #[test]
    fn macos_access_failures_are_classified_without_platform_details() {
        for code in [-25293, -128] {
            let error = security_framework::base::Error::from_code(code);
            assert_eq!(
                classify_platform_read_failure(&error),
                ReadonlySecretError::AccessDenied
            );
        }
        for code in [-25308, -25291, -25292, -25294, -25295] {
            let error = security_framework::base::Error::from_code(code);
            assert_eq!(
                classify_platform_read_failure(&error),
