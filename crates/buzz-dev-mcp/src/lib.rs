...
impl DevMcp {
    ...
    #[tool(
        name = "shell",
        description = "Run a command in the configured shell (bash by default; `BUZZ_SHELL` can select another). Each call starts a new process. Output is tail-truncated for the model and the full captured output is saved as an artifact. `timeout_ms` defaults to 120000 and is capped at 600000. Use a longer timeout for builds, tests, and pushes."
    )]
    async fn shell(
        &self,
