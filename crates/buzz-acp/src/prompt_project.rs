...
pub fn pick_listed_project_home(events: &[Value]) -> Option<PromptProjectInfo> {
    let mut listed: Vec<&Value> = events
        .iter()
        .filter(|event| !event_is_unlisted(event))
        .collect();
...
mod tests {
    ...
    fn project_event(
        ...
        let mut tags = vec![
            json!(["d", slug]),
            json!(["name", name]),
            json!(["buzz-channel", "11111111-1111-4111-8111-111111111111"]),
        ];
    ...
    fn pick_listed_project_home_prefers_oldest_listed() {
        ...
        let home = pick_listed_project_home(&events).expect("listed project");
