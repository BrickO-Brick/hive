...
export function BackupStep({
  ...
  const storageDescription =
    identityStorage === "system-keyring"
      ? "Hive keeps your identity key in your system keychain. Your computer may ask for your password when Hive needs to read the key."
      : identityStorage === "local-file"
        ? "Your system keychain wasn’t available, so Hive keeps your identity key in a private file on this device."
        : "Hive keeps your identity key protected on this device. Make a separate backup in case you lose access.";
  const storageTitle =
    identityStorage === "system-keyring"
  ...
  const introStorageDescription =
    identityStorage === "system-keyring"
      ? "Hive keeps your identity key in your system keychain."
      : identityStorage === "local-file"
        ? "Hive keeps your identity key in a private file on this device because the system keychain wasn’t available."
      ...
      >
        <div className="flex w-full max-w-140 shrink-0 flex-col text-center">
          ...
          <p className="mt-5 text-sm leading-6 text-foreground/75">
            Your identity key works like a password for your Hive account. Keep
