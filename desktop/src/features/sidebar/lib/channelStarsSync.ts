...
export class ChannelStarSyncManager {
  ...
  // Monotonic id for the current pending edit. Every publishStars() bumps it;
   ...
   */
  observe(store: ChannelStarStore): void {
  ...
  private async doPublish(store: ChannelStarStore, gen: number): Promise<void> {
  ...
  destroy(): void {
    ...
    // Debounce-window changes are NOT lost: publishStars persisted them to the
