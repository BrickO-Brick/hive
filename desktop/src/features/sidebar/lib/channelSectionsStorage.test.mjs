...
test("readChannelSectionsStore: scoped-write failure returns DEFAULT and leaves neither relay able to seed the legacy value", () => {
  const pubkey = "pk-migrate-writefail";
  const relayA = "wss://relay-a-writefail.example.com";
  const relayB = "wss://relay-b-writefail.example.com";
  const legacy = makeStore({
    sections: [makeSection({ id: "sw", name: "Legacy", order: 0 })],
    assignments: {},
  });
  ...
  try {
    ...
    assert.deepEqual(readChannelSectionsStore(pubkey, relayA), DEFAULT_STORE);
  ...
  assert.deepEqual(readChannelSectionsStore(pubkey, relayB), legacy);
...
test("readChannelSectionsStore: a legacy delete that does not take rolls back and returns DEFAULT", () => {
  const pubkey = "pk-migrate-delfail";
  const relay = "wss://relay-delfail.example.com";
  const legacy = makeStore({
    sections: [makeSection({ id: "sd", name: "Legacy", order: 0 })],
    assignments: {},
  });
  writeChannelSectionsStore(pubkey, legacy);
  const legacyKey = storageKey(pubkey);
...
test("readChannelSectionsStore: legacy delete succeeds but the confirmation read throws — scoped copy retained, no data loss", () => {
  const pubkey = "pk-migrate-confirmthrow";
  const relay = "wss://relay-confirmthrow.example.com";
  const legacy = makeStore({
    sections: [makeSection({ id: "sc", name: "Legacy", order: 0 })],
    assignments: {},
  });
  writeChannelSectionsStore(pubkey, legacy);
  const legacyKey = storageKey(pubkey);
...
test("readChannelSectionsStore: legacy delete throws while the probe stays healthy — scoped rollback, DEFAULT, single future claimant", () => {
  const pubkey = "pk-migrate-delthrow-probeok";
  const relay = "wss://relay-delthrow-probeok.example.com";
  const legacy = makeStore({
    sections: [makeSection({ id: "st", name: "Legacy", order: 0 })],
    assignments: {},
  });
  writeChannelSectionsStore(pubkey, legacy);
  const legacyKey = storageKey(pubkey);
...
test("readChannelSectionsStore: legacy delete and the catch probe both throw — scoped kept but hidden while legacy remains", () => {
  const pubkey = "pk-migrate-delthrow-probethrow";
  const relay = "wss://relay-delthrow-probethrow.example.com";
  const legacy = makeStore({
    sections: [makeSection({ id: "sb", name: "Legacy", order: 0 })],
    assignments: {},
  });
  writeChannelSectionsStore(pubkey, legacy);
  const legacyKey = storageKey(pubkey);
