...
test("same-second star and unstar mutations survive at capacity", async () => {
  ...
  try {
    for (const [pubkey, action, expectedStarred] of [
      ...
      assert.equal(persisted.channels["a-target"].starred, expectedStarred);
      unmount();
    }
  } finally {
    cleanup();
...
// future-stamped remote (updatedAt = t+300, rev 7, unstarred), a slow device
...
test("fast-clock veto fix: click after observing a future-stamped head wins", async () => {
  const { act, cleanup, renderHook } = await import("@testing-library/react");
  const { relayClient } = await import("@/shared/api/relayClient");
  const { readChannelStarsStore } = await import("./channelStarsStorage.ts");
  const { useChannelStars } = await import("./useChannelStars.ts");
    ...
    invoke: (cmd) => {
      if (cmd === "nip44_decrypt_from_self")
        return Promise.resolve(
          starPayload({ shared: { starred: false, updatedAt: 400, rev: 7 } }),
        );
...
test("far-future observation: timestamp stays fixed, rev advances, latest click wins", async () => {
  ...
  window.__TAURI_INTERNALS__ = {
    invoke: (cmd) => {
      if (cmd === "nip44_decrypt_from_self")
        return Promise.resolve(
          starPayload({ shared: { starred: true, updatedAt: FUTURE, rev: 1 } }),
        );
      return Promise.reject(new Error(`unmocked ${cmd}`));
    },
  };
  ...
  window.__TAURI_INTERNALS__ = {
    invoke: (cmd) => {
      if (cmd === "nip44_decrypt_from_self")
        ...
        return Promise.resolve(
          starPayload({ shared: { starred: false, updatedAt: 500, rev: 99 } }),
        );
      return Promise.reject(new Error(`unmocked ${cmd}`));
    },
  };
...
test("bootstrap resumes a persisted outbox edit", async () => {
  ...
  const outboxKey = `buzz-channel-stars-outbox.v1:${pubkey}:${encodeURIComponent(relayUrl)}`;
  window.localStorage.setItem(
    outboxKey,
    starPayload({ resumed: { starred: true, updatedAt: 90, rev: 2 } }),
  );
  let hook = null;
  try {
    await act(async () => {
