...
pub struct ManagedAgentRecord {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub catalog_source: Option<CatalogSource>,
    /// Cross-workspace library linkage (§2.6). `Some(library_id)` marks this
    /// keyless definition as a projection of a shared library entry — the
    /// mutation seam (§2.7) routes edits/deletes of such a record through the
    /// library machinery instead of the plain local path, and the read seam
    /// (`load_persona_views`) surfaces it as a shared indicator. Only §3's
    /// library operations author this; no command input or inbound event may.
    /// `#[serde(default, skip_serializing_if)]` keeps records without it
    /// byte-identical to head.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub library_ref: Option<String>,
    /// The library-entry revision this projection's cached content reflects
    /// (§2.6). Paired with `library_ref`; meaningless without it. Written only
    /// by §3's `apply_shared_definition`/materialization; never round-tripped
