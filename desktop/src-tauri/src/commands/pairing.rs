    ...
    tokio::task::spawn_blocking(move || {
        ...
        commit_recovery_if_current(&generation, &generation_fence, task_generation, || {
            ...
            crate::commands::identity::commit_imported_identity(&state, &data_dir, keys, |keys| {
                ...
                // agent save/GC on the same SecretStore, held across the persist
                // span only. Lock order on this path is identity_mutation →
                // pairing generation_fence → txn; no save/GC path touches either
                // of the first two, so there is no inversion.
