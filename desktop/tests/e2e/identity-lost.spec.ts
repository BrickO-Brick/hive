...
test("normal first launch uses the already-persisted identity", async ({
  ...
  await expect(
    page.getByRole("button", { name: "Sign in with Mantap" }),
...
test("lost boot offers phone recovery with a single-use QR", async ({
  ...
  await expect(
    page.getByText("Scan this code with a signed-in Hive phone."),
  ).toBeVisible();
  await expect(
...
test("phone recovery uses the desktop pairing card semantics", async ({
  ...
  await expect(
    card.getByText(
      "This gives this desktop permanent access to your Hive identity. Only continue if you trust it.",
    ),
  ).toBeVisible();
  await expect(
