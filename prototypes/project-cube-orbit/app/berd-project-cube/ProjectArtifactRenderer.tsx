...
function PrototypeCube({
  ...
  const focusRotation = useRef({ x: 0, y: 0 });
  const previousFocusRotation = useRef({ x: 0, y: 0 });
  ...
  useFrame((frameState, delta) => {
    ...
    if (groupRef.current) {
      ...
      const focusTargetX = focusSide ? -0.1 : 0;
      const focusTargetY = focusSide ? 0.68 : 0;
      focusRotation.current.x +=
        (focusTargetX - focusRotation.current.x) * 0.075;
      focusRotation.current.y +=
        (focusTargetY - focusRotation.current.y) * 0.075;
      const focusDeltaX =
        focusRotation.current.x - previousFocusRotation.current.x;
      const focusDeltaY =
        focusRotation.current.y - previousFocusRotation.current.y;
      previousFocusRotation.current.x = focusRotation.current.x;
      previousFocusRotation.current.y = focusRotation.current.y;

      groupRef.current.rotation.y +=
        force * antiFlatStrength + tileSpinDeltaY + focusDeltaY;
      groupRef.current.rotation.x += tileSpinDeltaX + focusDeltaX;
...
const SILHOUETTE_SAMPLE_INTERVAL = 1 / 60;

type ScreenPoint = { x: number; y: number };

function projectedConvexHull(points: ScreenPoint[]): ScreenPoint[] {
...
function CubeProjectionReporter({
  shellRef,
  onCubeProjection,
}: {
  shellRef: MutableRefObject<THREE.Mesh | null>;
  onCubeProjection?: (projection: ProjectArtifactProjection) => void;
}) {
  const lastSampleTimeRef = useRef(Number.NEGATIVE_INFINITY);
  const projectedVertexRef = useRef(new THREE.Vector3());

  useFrame(({ camera, size, clock }) => {
    const shell = shellRef.current;
    const geometry = shell?.geometry;
    if (!shell || !geometry || !onCubeProjection) return;
    if (
      clock.elapsedTime - lastSampleTimeRef.current <
      SILHOUETTE_SAMPLE_INTERVAL
    ) {
      return;
    }
    lastSampleTimeRef.current = clock.elapsedTime;

    shell.updateWorldMatrix(true, false);
    const position = geometry.getAttribute("position");
    if (!position) return;
    const projectedVertices: ScreenPoint[] = [];
    const projectedVertex = projectedVertexRef.current;
    ...
    const contour = projectedConvexHull(projectedVertices);
  ...
  return (
    <>
      ...
      <PrototypeCube
        ...
        variant={variant}
      />
      <CubeProjectionReporter
        shellRef={shellRef}
        onCubeProjection={onCubeProjection}
      />
      <CubeInteraction
        cameraAnglesRef={cameraAnglesRef}
        runtimeRef={runtimeRef}
  ...
  return (
    <div
      ...
      className={cn(
        ...
        variant === "tile"
          ? "overflow-visible bg-transparent cursor-grab active:cursor-grabbing [transform:translateZ(0)]"
