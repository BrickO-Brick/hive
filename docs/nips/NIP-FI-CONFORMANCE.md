...
|---|---|---|---|
| `FI-TRACE-PROXY-SPOOF` | Direct ingress, v1 envelopes, missing or repeated provenance or client-peer fields, mixed profiles, client-supplied fields, malformed canonical encoding, and every clock or size violation deny before authority. No fallback to `client-attached` occurs. | Deployed negatives from inside and outside the edge; v1 rejection; missing/repeated/comma/extra components; canonical and non-canonical IPv4, IPv6, and IPv4-mapped peer cases; non-canonical base64url; nonce lengths 15, 16, configured maximum, and maximum plus one; MAC lengths 31, 32, and 33; field maximum and maximum plus one; future-skew equality and excess; age-bound just-before and equality; overflow cases; listener topology; responses; and no-mutation state diffs. | `FI-INV-04`, `FI-INV-05`, `FI-INV-14` |
| `FI-TRACE-PROXY-REPLAY` | Final admissions using one live HMAC nonce commit at most once, including when the nonce is re-signed by another concurrently active secret. Preparation consumes nothing. | Parallel same-secret and cross-secret overlap transcripts, domain/profile replay namespace, replay-store before/after state, committed receipt count, matched secret version as private audit metadata only, and retention deadline. | `FI-INV-08`, `FI-INV-09`, `FI-INV-14` |
| `FI-TRACE-PROXY-CROSS-REQUEST` | Changing assertion, authorization domain, proof transport, authenticated client peer, method, authority, path/query, or body denies under the original MAC. | One valid baseline plus one mutation for every bound field, exact canonical byte fixtures for the `NIP-FI-PROXY-2` input, and wire results. | `FI-INV-04`, `FI-INV-05` |
...
| `FI-TRACE-VERIFIER-PARITY` | After valid profile-specific transport handling, the same assertion, policy, time, and JWKS snapshot produce the same normalized assertion-verifier decision on every transport. Every configured or compiled assertion-semantic change advances `policy_id`; rotating only JWKS contents does not. WebSocket and HTTP accept the assertion only as exactly one `Nostr-Federated-Identity: Bearer <JWT>` field. | Shared assertion corpus through every adapter with byte-for-byte normalized verifier decisions; deterministic vectors that mutate issuer, audience, algorithm set, authenticated key-source identity, claim rules, time and size bounds, normalization rules, and compiled contract version one at a time; key-add/remove/order-only vectors; cross-process reproducibility; valid single-field cases; and transport rejection of `Authorization` bearer assertions, missing/repeated/comma-combined fields, provenance on `client-attached`, and mixed profiles on both WebSocket and HTTP. | `FI-INV-06`, `FI-INV-16` |
...
| `FI-TRACE-DEPENDENCY-FAIL-CLOSED` | Unreadable verifier, JWKS, binding, lifecycle, replay, policy, receipt, audit, or invalidation state never allows. | One injected outage per dependency at preparation, final admission, and lease reuse where applicable; results and state diffs. | `FI-INV-14` |
...
- proxy traces may be not applicable only when `trusted-proxy-hmac-v2` is neither accepted nor advertised and executable absence cases show rejection;
...
4. Trusted-proxy deployments include live bypass, mixed-profile, header-injection, replay, and cross-request evidence.
